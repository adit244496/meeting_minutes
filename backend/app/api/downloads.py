...
def export_meetings(
    ...
    with zipfile.ZipFile(buffer, "w", zipfile.ZIP_DEFLATED) as archive:
        for meeting in meetings:
            if kind in ("transcript", "both"):
                ...
                if rows:
                    text = _transcript_text(meeting, rows, _speaker_names(db, meeting.id))
                    folder = "transcripts/" if kind == "both" else ""
                    archive.writestr(folder + unique(_filename(meeting, "txt", "transcript")), text)
                    written += 1
    ...
    filename = f"neo-minutes-{label}-{span}-{datetime.now().strftime('%Y-%m-%d')}.zip"
...
def download_transcript(
    ...
    if fmt == "docx":
        return _attachment(
            _transcript_docx(meeting, rows, names), _filename(meeting, "docx", "transcript"), DOCX_TYPE
        )

    if fmt == "txt":
        return _attachment(
            _transcript_text(meeting, rows, names),
            _filename(meeting, "txt", "transcript"),
...
def _transcript_docx(meeting: Meeting, rows: list[Segment], names: dict[str, str]) -> bytes:
    def build(doc) -> None:
        doc.add_heading(meeting.title, level=0)
        if meeting.started_at:
